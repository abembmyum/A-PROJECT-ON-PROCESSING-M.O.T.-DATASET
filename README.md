...
## 8.2 Conclusion
As part of the project,  different technologies are explored and utilized for creating an end-to-end data pipeline to ingest M.O.T. datasets. Furthermore, some data cleaning and preprocessing were performed. Terraform was deployed to manage the infrastructure i.e., the Google Cloud Platform (GCP) where the Cloud Storage (data lake) and BigQuery (data warehouse) are hosted. Docker was also utilized to host the airflow. Airflow builds, orchestrates and monitors the workflow of the data pipeline. For the editor, Visual Code Studio (VSCode) was employed to create and manage the script files (.py,.tf,.yml). Lastly, for creating the dashboard, the Google Data Studio is utilized in order to display our finding and answer the questions.
